...
import { PARAPHRASE_MODE_IDS } from "@/lib/paraphrase-modes";
...
export async function POST(req: NextRequest) {
  ...
  const access = await checkFeatureAccess(auth.session.user.id, "paraphrase");
  ...
  try {
    ...
    return apiError(err instanceof Error ? err.message : "Paraphrase failed", 500);
  }
}
