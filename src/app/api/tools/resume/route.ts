...
import { runResumeBuilder } from "@/lib/run-writing-tool";
...
export async function POST(req: NextRequest) {
  ...
  const access = await checkFeatureAccess(auth.session.user.id, "resume-builder");
  ...
  try {
    const result = await runResumeBuilder(parsed.data as Record<string, string>);

    await logAIRequest({
      userId: auth.session.user.id,
      endpoint: "/api/tools/resume",
    ...
    return apiError(err instanceof Error ? err.message : "Resume generation failed", 500);
  }
}
